...
class YtForm extends Model {
    ...
    public function rules()
    {
        return [
            ...
            ['type', 'in', 'range'  => array_keys(Order::typeLabelsYt())],
            ...
            ['link', 'match', 'pattern' => '/youtu\.be|youtube.com/i', 'message' => Yii::t('app','Please, enter YouTube URL')],


            /*
            [['link'], 'match', 'pattern' => '/youtu\.be\/([a-z0-9-_]+)|v=([a-z0-9-_]+)|\/v\/([a-z0-9-_]+)|\/embed\/([a-z0-9-_]+)/i',
                'message' => Yii::t('app','Введите адрес видео YouTube или измените тип заказа'),
                'whenClient' => $condClientVideoLink,
                'when' => function($model){ return ( $model->type == Order::TYPE_LIKE or $model->type == Order::TYPE_DISLIKE); }
            ],
            [['link'], 'match', 'pattern' => '/\/channel\/([a-z0-9-_]+)|com\/(?:c\/)?([а-яa-z0-9-_]+)$|\/user\/([a-z0-9-_]+)/iu',
                'message' => Yii::t('app','Введите адрес канала YouTube или измените тип заказа'),
                'whenClient' => $condClientChannelLink,
                'when' => function($model){ return ( $model->type == Order::TYPE_SUBSCRIBE); }
            ],
            */

            [['link'], 'string', 'length' => [ 25, 95 ]],
    ...
    public function checkBalance() {
        ...
        $total += $prices['yt'][$this->type] / 1000;
