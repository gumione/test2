...
class InstagramForm extends Model {
    ...
    public $gender;
    public $insurance;
    public $social = Order::SOCIAL_INSTAGRAM;
    // -- цена заказа
    ...
    public function rules() {
        ...
        return [
            ...
            ['type', 'in', 'range' => array_keys(Order::typeLabelsInsta())],
            ...
            [['link'], 'trim'],
            ['link', 'filter', 'filter' => [$this, 'fixLink']],
            [['link'], 'match', 'pattern' => '/((?:(?:http|https):\/\/)?(www.)?instagram\.com\/(.)+)|(([A-Za-z0-9-_\.]+))/i', 'message' => Yii::t('app', 'Please enter Instagram URL')],

            ['age', 'in', 'range' => array_keys(Order::ageLabels()), 'message' => Yii::t('app', 'Invalid age')],
    ...
    public function run() {
        ...
        if ($this->validate()) {
            ...
            $order->social = Order::SOCIAL_INSTAGRAM;
    ...
    public function checkBalance() {
        ...
        $total += $prices['insta'][$this->type] / 1000;
...
//        } else {
    ...
    /**
     * Фильтр исправляющий ссылку (может приходить только имя акка)
     * @param $value
     * @return string
     */
    public function fixLink($value) {
