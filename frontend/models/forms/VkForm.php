...
class VkForm extends Model 
{
    ...
    public function rules() 
    {
        ...
        return [
            ...
            ['gender', 'default', 'value' => Order::INSURANCE_MINIMUM],
            
            ['type', 'in', 'range'  => array_keys(Order::typeLabelsVk())],
            ...
            [['link'], 'trim'],
            [['link'], 'url', 'message' => Yii::t('app','Введите правильную ссылку')],
            [['link'], 'match', 'pattern' => '/https?\:\/\/(www.)?vk\.com\/(.)+/i', 'message' => Yii::t('app','Введите адрес ссылки VK')],

//            [['link'], 'wordFilter', 'when' => function( $model ) {
//                $passlink = Yii::$app->session->get('passlink', false);
//                $linktime = 0;
//                if ( $passlink ) {
//                    $linktime = explode('|||', $passlink)[1];
//
//                }
//                return ( (time()-$linktime)>=30 );
//            }],
    ...
    public function run() {
        ...
        if ( $this->validate() ) {
            ...
            $type = Yii::$app->session->get('vkOrderType', false);
            $order->social = Order::SOCIAL_VK;
    ...
    public function checkBalance() {
        ...
        $total += $prices['vk'][$this->type] / 1000;
