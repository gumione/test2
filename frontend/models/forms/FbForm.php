...
class FbForm extends Model {
    ...
    public function rules()
    {
        ...
        return [
            ...
            ['type', 'in', 'range'  => array_keys(Order::typeLabelsFb())],
            ...
            [['link'], 'trim'],
            [['link'], 'url', 'message' => Yii::t('app','Please enter correct URL')],
            [['link'], 'match', 'pattern' => '/https?\:\/\/(www\.|m\.)?facebook\.com\/(.+)/i', 'message' => Yii::t('app','Please enter Facebook URL')],
            [['link'], 'string', 'length' => [ 28, 350 ]],
            [['link'], 'filter', 'filter' => [ $this, 'linkPure' ] ],
    ...
    public function checkBalance() {
        ...
        $total += $prices['fb'][$this->type] / 1000;
        ...
        if($this->insurance != Order::INSURANCE_MINIMUM) {
            ...
            $total += round($total/100*$rates[$this->insurance]);
        }
//        
//        if ( $coins < $total ) {
...
//        } else {
    ...
    /**
     * Фильтр упрощающий ссылку, если это необходимо.
     * + перевод на мобильную версию для ускорения работы.
     * @param $value
     */
    public function linkPure ( $value ) {


        /**
         * Переводим ссылки на мобильную версию с www
         */
        $value = str_replace(['//www.', '//facebook.com'], ['//m.', '//m.facebook.com'], $value);

        /**
         * Упрощаем ссылки вида:
         * https://www.facebook.com/groups/307942079410834/?multi_permalinks=687718174766554&notif_t=commerce_int...
         */
        if ( preg_match( '@groups/(\d+)/\?multi_permalinks=(\d+)@i', $value, $result ) ) {
            return "https://m.facebook.com/groups/{$result[1]}/permalink/{$result[2]}/";
        }

        /**
         * Упрощаем ссылки вида:
         * https://m.facebook.com/groups/1417265381899959?view=permalink&id=1769686033324557&refid=18&_ft_=qid.64...
         */
        if ( preg_match( '@groups/(\d+)\?view=permalink&id=(\d+)@i', $value, $result ) ) {
            return "https://m.facebook.com/groups/{$result[1]}/permalink/{$result[2]}/";
        }
